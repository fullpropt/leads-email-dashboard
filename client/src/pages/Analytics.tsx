...
export default function Analytics() {
  ...
  // Buscar dados de jornada do usuário
  const { data: journeyData, isLoading: isLoadingJourney, refetch: refetchJourney } = trpc.tubetools.getUserJourneyAnalytics.useQuery(
    undefined,
    {
      staleTime: 30000,
      refetchOnWindowFocus: false,
  ...
  if (isLoading || isLoadingTemporal || isLoadingJourney) {
    return (
        ...
        <TabsContent value="overview" className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">${stats.avgBalance.toFixed(2)}</div>
                <p className="text-xs text-muted-foreground">
            ...
            {/* Resets por Inatividade */}
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Resets por Inatividade</CardTitle>
                <RefreshCw className="h-4 w-4 text-orange-600" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold text-orange-600">{stats.totalResets || 0}</div>
                <p className="text-xs text-muted-foreground">
                  {stats.usersWithResets || 0} usuários afetados
                </p>
              </CardContent>
            </Card>
