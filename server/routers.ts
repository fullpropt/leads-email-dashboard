...
export const appRouter = router({
  ...
  router.post("/webhooks/newSignup", async ({ input }) => {
    ...
    // Usar o mesmo processamento do webhook de compra, mas com leadType = "novo_cadastro"
    ...
    if (existingLead.length === 0) {
      ...
      await db.insert(leads).values({
        ...
        emailEnviado: 0,
        status: "active",
        leadType: "novo_cadastro", // ← IMPORTANTE
        isNewLeadAfterUpdate: 1,
      });
      
      console.log(`[Router] Novo lead criado: ${email}`);
    }
    ...
    // Enviar email automático se ativado
    ...
    if (autoSendEnabled) {
      const templatesWithAutoSend = await getTemplatesByTypeAndSendType("novo_cadastro", "immediate");
      ...
      if (templatesWithAutoSend.length > 0) {
        ...
        for (const template of templatesWithAutoSend) {
          try {
            ...
            if (emailSent) {
              await updateLeadEmailStatus(lead.id, true);
              console.log(`[Router] Email enviado para novo cadastro: ${email}`);
            }
          } catch (error) {
            console.error(`[Router] Erro ao enviar email:`, error);
          }
        }
      }
    }
    ...
    return {
      success: true,
      message: "Novo cadastro processado com sucesso",
      email: email,
    };
  });
});
